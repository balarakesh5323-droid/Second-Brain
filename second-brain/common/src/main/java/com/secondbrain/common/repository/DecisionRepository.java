...
public interface DecisionRepository extends JpaRepository<Decision, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT d FROM Decision d LEFT JOIN FETCH d.project LEFT JOIN FETCH d.repository WHERE (:after IS NULL OR d.createdAt > :after OR (d.createdAt = :after AND d.id > :lastId)) ORDER BY d.createdAt ASC, d.id ASC")
