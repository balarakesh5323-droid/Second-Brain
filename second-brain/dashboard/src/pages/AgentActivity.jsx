...
import { Clock, Bot, GitCommit } from 'lucide-react';

export default function AgentActivity() {
  const { data: events } = useQuery({
    queryKey: ['events'],
    queryFn: () => brainApi.getRecentEvents().then(r => r.data),
  });

  const { data: sessions } = useQuery({
    queryKey: ['sessions'],
    queryFn: () => brainApi.getRecentSessions().then(r => r.data),
  });
          ...
          {sessions?.map((session) => (
            <div key={session.id} className="flex items-center gap-4 p-4 bg-gray-800 rounded-lg">
              ...
              <div className="flex-1">
                <p className="font-medium">{session.agent?.name || 'Unknown Agent'}</p>
                <p className="text-sm text-gray-400">{session.task}</p>
              </div>
